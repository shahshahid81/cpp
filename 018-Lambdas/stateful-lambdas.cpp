...
void test1()
{
  ...
  // we have captured the value of local_x by value so a copy is used in the lambda body and value is const by default so we cannot modify local_x
  auto l = [local_x]()
  {
...
void test2()
{
  ...
  // we have captured the value of x by value so a copy is used in the lambda body and value is mutable so we can modify x
  // also, the value is captured once only, so further method calls won't re capture the value.
  // Internally, compiler generates a class with some name and whatever value we pass is sent to constructor. hence value is captured only once.
...
void test4()
{
  ...
  // [=] means to capture all the variable used in lambda body by copying them which is default
  auto l = [=]() mutable
  {
    x += 100;
    y += 100;
...
void test5()
{
  ...
  // [&] means to capture all the variable used in lambda body by creating references
  auto l = [&]()
  {
    x += 100;
    y += 100;
...
void test6()
{
  ...
  // here, after specifying copy for all the variables, we are overwriting copy of y with reference of y. So all other variables are copyed by value and y is referenced
  auto l = [=, &y]() mutable
  {
    x += 100;
    y += 100;
...
void test7()
{
  ...
  // here, after specifying reference for all the variables, we are overwriting reference of x and z with copy of x and z. So all other variables are referenced and x and y are copied.
  auto l = [&, x, z]() mutable
  {
    x += 100;
    y += 100;
...
public:
  ...
  std::vector<Person> get_people(int max_age)
  {
    ...
    std::copy_if(people.begin(),
                 ...
                 //  Capturing the this context, count reference and copy of max age
